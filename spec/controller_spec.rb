...
describe Controller do
  ...
  describe '#activate_rovers' do
    ...
    describe 'in bounds' do
      ...
      it 'returns output of third rover after second rover marks its beacon' do
    ...
    describe 'out of bounds' do
      context 'in y plane' do
        it 'returns output if next move is out of upper bound' do
          input = [[[1, 4, 'N'], ['M', 'M']]]
          allow(inputs).to receive(:parse_text).and_return(input)
          expect(controller.activate_rovers).to eq(['1 5 N RIP'])
        end

        it 'returns output if next move is out of lower bound' do
          input = [[[1, 1, 'S'], ['M', 'M']]]
          allow(inputs).to receive(:parse_text).and_return(input)
